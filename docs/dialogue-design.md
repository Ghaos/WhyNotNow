...
ローカルのブラウザダッシュボードは、タスク本文を素早く捕捉し、実行前・実行中・完了を見直し、開始方式を選ぶ正規のレビュー面です。Codex は解釈、調査、編集、実作業、アーカイブのためのワークベンチです。両者の責務、共有するデータ正本、代替導線の詳細は、[AI と GUI の境界](interaction-boundary.md)で定義します。

会話の中で、アシスタントが解消を助けられる小さく限定的な障害が見つかることがあります。その支援は具体的かつ有用な場合にのみ提案し、すべての理由を調査依頼に変えてはいけません。

## フロー

全体の流れは [対話フローチャート](dialogue-flowchart.md) を参照してください。`$wnn` は最初のアクションフォームを表示せず、Why-not-now対話へ進みます。ダッシュボードから開始した場合も、クリック済みの選択をCodexで尋ね直しません。対話が進んだ後に判断を変更するときだけアクションフォームを使います。
