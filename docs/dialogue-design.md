...
ローカルのブラウザダッシュボードは、タスク本文を素早く捕捉し、実行前・検討中・実行済みを見直し、次の行動を選ぶ面です。Codex は理由の解釈、限定調査、構造化情報の更新、実作業を担います。セッションIDやリンクは共有せず、両者の責務は [AI と GUI の境界](interaction-boundary.md)で定義します。

会話の中で、アシスタントが解消を助けられる小さく限定的な障害が見つかることがあります。その支援は具体的かつ有用な場合にのみ提案し、すべての理由を調査依頼に変えてはいけません。

## フロー

全体の流れは [対話フローチャート](dialogue-flowchart.md) を参照してください。`$wnn` は検討中の項目を作り、Why-not-now対話へ進みます。ダッシュボードの **Why not now?** と **Do it now** はそれぞれ新しいCodexセッションを起動し、クリック済みの選択を尋ね直しません。対話中に実行したくなった場合は、ダッシュボードの **Do it now** から別セッションを起動します。
